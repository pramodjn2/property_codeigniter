...
<div class="main-container inner_maincontainer" style="margin-top:0px !important;">
  ...
  <?php $this->load->view('listings/latest_property_listing'); ?>
...
<script type="text/javascript" src="<?php echo config_item('base_url');?>assets/js/gmap/property_map.js"></script> 
...
/*$( "#location" ).change(function() {
	...
	$( "#howmanyrooms" ).change(function() {
	 ...
	 var howmanyrooms = $( "#howmanyrooms" ).val();
	 
	 if(howmanyrooms=='Sale'){
	 
	  window.location=base_url+"property/forsale";
	 
	 }else{
	  window.location=base_url+"property/forrent";
	 }
	...
	function ajaxPaging(url){
		...
		if(location == ''){
			...
			if(mt=='yes'){				
				...
				var url = base_url+'ajax/listing/'+res[ln];
  ...
  var filename = server_url.substring(server_url.lastIndexOf('/')+1);
		...
		/*	 https://otriga.com/property/listing?location=United+Kingdom&near_by=&min_price=10000&max_price=6500000&bed_room=&property_type=&proprty_category=Sale&city=&regions=&country_code=GB&postal_code=*/
...
var url = base_url+'property/'+filename+'?location='+location+'&near_by=&min_price='+min_price+'&max_price='+max_price+'&bed_room=&property_type='+property_type+'&proprty_category='+proprty_category+'&city='+city+'&regions='+regions+'&country_code='+country+'&postal_code='+postal_code+'';
