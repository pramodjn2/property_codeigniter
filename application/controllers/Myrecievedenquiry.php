...
class Myrecievedenquiry extends CI_Controller {   
    ...
    {	
		...
		$crud->where('free_valuation.receiver_id', $user_id);
		$crud->order_by('free_valuation.id', 'desc');	    
      
	  	
		
		$crud->unset_fields('user_id','property_id','sender_id','receiver_id','message_status');

	    $crud->columns('property_link','uname', 'email','cell_number', 'date_time');
		...
		$output = $crud->render(); 
		$page_title = array('page_title'=>'Manage Recieved Enquiry');
	...
	function _property_link($primary_key , $row){ 
	 ...
	 // $getData = $query->result_();
	  ...
	  return '<a href="'.base_url($detailUrl.$property_seo_url).'" class="crud-action propertyEnq" title="'.$property_name.'" target="_blank" style="text-decoration:underline; color:blue !important;">'.$property_name.'</a>';
	}
