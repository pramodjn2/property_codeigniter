	...
	public function getPaymentStatus( $ID = 0 ){
 	...
 	public function payWithPaypal(){
		...
		if (empty($_REQUEST['user_id']) || $_REQUEST['user_id'] <= 0 || 
			empty($_REQUEST['membership_plan_id']) || $_REQUEST['membership_plan_id'] <= 0){ Redirect(); return; }
		
		$dbresult = $this->common->select('membership_plan', " where plan_id = " . $_REQUEST['membership_plan_id']);
		if (empty($dbresult) || count($dbresult) <= 0){
			Redirect(); return;
		}
		$planamount = $dbresult[0]['price'];
		$durationindays = $dbresult[0]['durationindays'];
		$plan_name = $dbresult[0]['plan_name'];
		
		
		$discount=getdiscountprice($dbresult[0]['discount_type'],$dbresult[0]['discount_price'],$dbresult[0]['price']);
		if(!empty($discount)){
		...
		$item_name			= $plan_name . ' - Membership Subscription ';
		...
		$data = array(
					...
					'mem_plan_id' 		=> $_REQUEST['membership_plan_id'],
		...
		if( $orderID ):
			...
			$paypalData['params'] = array(
									...
									'return_url'=> config_item('base_url'). 'Mypayment/returnurl/1',
									'cancel_url'=> config_item('base_url'). 'Mypayment/returnurl/cancel',
									'notify_url'=> config_item('base_url'). 'Mypayment/processIPN'
			);
