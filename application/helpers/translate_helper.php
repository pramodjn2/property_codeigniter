...
if ( ! function_exists('translate'))
{
	function translate($text, $d = '')
	{
		...
		if($d !='en_US'){
			...
			$url = "https://www.googleapis.com/language/translate/v2?q=House&target=ka&source=en&key=AIzaSyCrMv41LMXkfKFn6JsJE-Jq4EYDBZ6vhYM";
			// Make sure to set CURLOPT_REFERER because Google doesn't like if you leave the referrer out
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_REFERER, "http://otriga.com/dev/");
			$body = curl_exec($ch);
			curl_close($ch);
